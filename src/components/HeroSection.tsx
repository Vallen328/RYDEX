  ...
  return (
    <div className='relative min-h-screen w-full overflow-hidden'>
        ...
        <div className='relative z-10 min-h-screen flex flex-col items-center justify-center px-4 text-center'>
          ...
          {/* Book any Vehicle ke baad yeh wala animation dikhe */}
          <motion.p
            initial = {{ opacity: 0 }}
            animate = {{ opacity: 1 }} 
          ...
          {/* Now we need to make a button called Book Now. So on clicking it, we can book. */}
          <motion.button
            whileHover={{scale: 1.05}}
            whileTap={{scale:0.95}} 
            className='mt-12 px-10 py-4 bg-white text-black rounded-full font-semibold shadow-xl' onClick={onAuthRequired}
          >
            Book Now
          </motion.button>
        </div>
